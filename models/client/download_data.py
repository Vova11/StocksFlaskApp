    ...
    def get_data(self, granularity=15 * 60, num_days=30, filename=None, market="ETH-USD", cointime=0.5):
        if filename is None:
            ...
            while start_cx < end:
                ...
                data = self.api.get_product_historic_rates(
                    market, granularity=granularity, start=start_query, end=end_query)
                df = pd.DataFrame(
                    data, columns=["Time", "Low", "High", "Open", "Close", "Vol"])
            ...
            time.sleep(cointime)
            data = self.api.get_product_historic_rates(
                market, granularity=granularity)
            df = pd.DataFrame(
                data, columns=["Time", "Low", "High", "Open", "Close", "Vol"])
            df = df.reindex(index=df.index[::-1])
            df = df.reset_index()
            ...
            cut_point = df[df['Time2'] == frames[-1]
                           ['Time2'].iloc[-1]].index.values[0]
            ...
            final_df.to_csv(
                f"{market}_{start.strftime('%Y-%m-%d')}_{end.strftime('%Y-%m-%d')}.csv")
...
def main():
    ...
    cdd.get_data(granularity=granularity, num_days=num_days,
                 filename=filename, market=market, cointime=0.5)


if __name__ == '__main__':
    main()

c = CoinbaseDataDownloader(api_key, api_secret, api_passphrase, api_url)
df = c.get_data(granularity=900, market="BTC-USD", num_days=1)
df = c.get_data(filename="BTC-USD_2020-04-30_2020-05-30.csv")
